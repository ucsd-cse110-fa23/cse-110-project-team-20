 ...
 *
 * Handles how input from other parts of the application should be formatted in order to meet JSON
 * specifications. Also converts from server-appropriate formatted data to one used by local methods
 */
public class JSONRecipeRepository implements IRecipeRepository, ISharedRecipeRepository {
    ...
    private void
    initDatabase(String jsonFilePath)
    {
    ...
    {
        List<Recipe> recipes = getListOfRecipes();
        Recipe newRecipe = new Recipe(Optional.ofNullable(recipe.getTitle()).orElse(""),
    ...
    private String
    readFile(InputStream inputStream) throws IOException
    {
