 ...
 * 
 * This class will check the provided user information from MongoDB. If the account is new one, create one and allow to log in.
 * If the account does exist, check the password is matched. If the password is not matched, throw the incorrect password exception.
 */
public class MongoDBAccountService implements IAccountService {
    ...
    public boolean loginOrCreateAccount(String username, String password) throws IncorrectPassword {
        try (MongoClient client = MongoClients.create(config.getConnectionString())) {
            MongoCollection<Document> accountCollection = client.getDatabase("pantrypal").getCollection("accounts");
            ...
            if (user != null) {
                ...
                // if the password is not matched, throw incorrect password error. Otherwise, logged in
