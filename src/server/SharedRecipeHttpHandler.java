 ...
 * 
 * if we have shared recipe under given shared_url, display the recipe. Otherwise, show not found page.
 */
public class SharedRecipeHttpHandler extends HttpHandlerBase {
  ...
  private String recipePage(Recipe recipe) {
  ...
  private String loadTemplate(String filename) {
    ...
    try {
      ...
      fileContent = String.join("", Files.readAllLines(Paths.get(getClass().getResource(filename).getPath())));
    } catch (IOException e) {
      e.printStackTrace();
